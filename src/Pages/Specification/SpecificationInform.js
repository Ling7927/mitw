...
const List=[
    ...
    {id:"6",title:"聯測網路與主機架構",content:<div>
        ...
        <ul>
            <il>實體面對面聯測者: 參加者連接聯測區域網路後，經過網路設定，將交易連結導向Gazelle Proxy主機作為抓取交易封包以及連結特定的測試單元。</il>
            <il>主機架設在雲端的參加單位: 透過VPN服務連線，設定外部主機之IP以及PORT將透過HTTP/HTTPS連線至連測區域網路內，GO Proxy作為外部主機IP與PORT轉換對照，用來擷取網路封包並記錄至Gazelle Proxy</il>
            <il>使用HTTP協定之情境: 驗證Level I時，須設定基本JWT Token作為基礎身分認證授權驗證，報名程序完成後，由主辦單位核發JWT Token,並將其放入HTTP Header中作為存取FHIR與DICOM主機認證使用。在互通性驗證時，雙方必須能支援動態身分授權機制，登入者身分須註冊到主辦單位提供的OAuth2主機，由參測主機端設定權限，交付督察員驗證時，須能證明雙方能採用標準化認證機制。</il>
            <il>使用DICOM協定之情境: 驗證Level I時，不需要設定JWT Token，但需透過DICOM AE 設定，以確保Client與Server主機能連線。</il>
        </ul>
