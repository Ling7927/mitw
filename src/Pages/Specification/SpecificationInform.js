...
const P1 = {
  ...
  context:
    "社團法人台灣醫療影像資訊標準協會 (MISAT)發佈的之醫療資訊規格以及公開技術文件，台灣醫學資訊聯測松工作小組基於HL7 FHIR®及DICOMWeb國際標準與規範，以標準化資訊互通介面，進行跨系統互通聯測(Connectathon)機制來驗證其互通性，企業會結盟並共同制定可互通性標準來建立標準化生態系(ecosystem)。針對每年舉辦的聯測松賽道徵求公開意見，請於點選 聯測工作小組選項，並註記賽道編號。",
  linkTitle: "公開徵求意見連結",
    ...
    content: (
      <div>
        ...
        <ul style={{ margin: "0" }}>
          <li>
            聯測工作小組會議:
            在面對面聯測活動前，針對不同賽道主題，分成多個標準工作小組，將招募會邀請國內醫療資訊標準專家(Domain
            Expert)、健康醫療領域專家等，並定期召集會議，確立、產生及公告規範。
          </li>
          <li>
            聯測準備活動(Pre-Connectathon)：
            面對面聯測前兩個月將提供線上測試資料以及測試主機，讓參加單位進行驗證，最為面對面聯測之準備。線上遇到問題時，將提供線上諮詢服務，並排除技術問題，另外會不定期提供線上讀書會、教育訓練以及意見交流活動。
          </li>
          <li>
            聯測工作坊(預演):
            為面對面聯測之前一個月舉辦一場聯測模擬測試，讓第一次或是不熟悉聯測工具以及流程的參加者能提前適應聯測。參加者將以實體參與活動，在一天的時間內對於參加賽道以及規格進行實際測試，活動將提供線上測試資料以及測試主機，讓參加單位進行驗證，最為面對面聯測之準備。另外會邀請部分督察員以及工作小組成員對於參加地的問題在現場排除問題，並提供諮詢。
          </li>
          <li>
            面對面聯測:
            參加單位將在四天內與其他參加單位以實際的網路連線進行系統介接測試，在四天內須完成指定的規範與測試資料，符合通過標準者，將提供產品通過證書。
          </li>
          <li>
            聯測松論壇：
            今年主題為:「打造標準化金融醫療生態圈」，預計邀請醫院、學術界以及政府單位專家進行多場專題演講，分享有關標準化金融醫療數據應用、政策，多元應用場景、核保理賠、健康照護/健檢以及智慧醫療科技串聯等領域的專題演講。
          </li>
          <li>
            成果展示：
    ...
    content: (
      <div>
        <p>
          ...
          Definition)。我們將聯測的分成10個測試整合規範。參加單位可由報名的測試整合規範，查看要符合此規範需要依循那些技術規格。在設計上，我們將此次的健康數據資料分成幾個領域
          (Domain): MI-TW以及RAD。
        </p>
      </div>
    ),
    ...
    content: (
      <div>
        <p>
          ...
          peer以及peer to
          peer這兩大類型。驗證規範由本會所招募的標準專家，將各賽道要驗證的測試情境定義在測試案例定義中。
        </p>
      </div>
    ),
  },
    ...
    content: (
      <div>
        ...
        <ul style={{ margin: "0" }}>
          ...
          <li>
            RAD為依循國際IHE規範Radiology中定義的各種整合規範(Integration
            Profile)
          </li>
        </ul>
      </div>
    ...
    content: (
      <div>
        <p>
          ...
          II為互通性驗證，要求測試系統需與三家不同的系統進行驗證，須通過各賽道的測試情境的測試腳本。Level
          III為連線測試時須支援動態身分授權機制，即系統需支援OAuth2認證功能，且雙方的系統需要均能支援。下表為驗測規則分級整理與說明。
        </p>
        <ul style={{ margin: "0" }}>
          <li>
            驗測規則分級整理表
          ...
          <li>
            提供OAuth2 Server，驗證參加單位OAuth2 Client，大會提供Keycloak
            Server作為認證主機。
          </li>
        </ul>
      </div>
    ...
    content: (
      <div>
        ...
        <ul style={{ margin: "0" }}>
          <li>
            實體面對面聯測者:
            參加者連接聯測區域網路後，經過網路設定，將交易連結導向Gazelle
            Proxy主機作為抓取交易封包以及連結特定的測試單元。
          </li>
          <li>
            主機架設在雲端的參加單位:
            透過VPN服務連線，設定外部主機之IP以及PORT將透過HTTP/HTTPS連線至連測區域網路內，GO
            Proxy作為外部主機IP與PORT轉換對照，用來擷取網路封包並記錄至Gazelle
            Proxy
          </li>
          <li>
            使用HTTP協定之情境: 驗證Level I時，須設定基本JWT
            Token作為基礎身分認證授權驗證，報名程序完成後，由主辦單位核發JWT
            Token,並將其放入HTTP
            Header中作為存取FHIR與DICOM主機認證使用。在互通性驗證時，雙方必須能支援動態身分授權機制，登入者身分須註冊到主辦單位提供的OAuth2主機，由參測主機端設定權限，交付督察員驗證時，須能證明雙方能採用標準化認證機制。
          </li>
          <li>
            使用DICOM協定之情境: 驗證Level I時，不需要設定JWT
            Token，但需透過DICOM AE 設定，以確保Client與Server主機能連線。
          </li>
        </ul>
        <p>
