...
const IndustryInform = [
  ...
  },
  {
    ...
    tracks:
      "參加賽道 \n TRACK#4 - DICOMWeb查詢與調閱 \n TRACK#5 - DICOM數位全幅病理顯微鏡影像查詢與調閱 \n TRACK#7 - 影像檢查流程",
    introduce:
      "UDE是EBM既有產品的轉型升級，將PACS的應用環境延伸到iPad Pro，整合為一個獨立的伺服器及顯示設備。 醫護人員在病患做完檢查後，將影像由儀器端傳至iPad Pro儲存與管理，可進行離線影像調閱瀏覽，也可做獨立工作站和報告系統整合等延伸應用。 UDE更可利用在本機的CoreML影像AI及雲端AI模組，將運算結果同步顯示在屏幕上，讓影像上呈現標註，輔助醫生判讀肉眼難以辨識的微小病灶。 未來也將導入更多影像AI模組，持續提升輔助醫師診斷效率及流程。",
  },
  {
    img: Advance,
    linkName: ["產品介紹", ""],
  ...
  },
  {
    ...
    introduce: (
      <>
        <ol>
          「健康小蜜蜂數位健康平台/HealthBee
          DHP」是補充醫療健康難及之處，提供健康管理機構及企業職場，聯合照護客戶與員工的生態環境，平台提供：
          <li>
            健檢機構1+364天的健康管理照護：平台提供全面數位化的客戶服務，節省每年龐大的郵寄、光碟等無意義浪費，升級客戶體驗全年貼身數位分身服務；健管師任務自動派工及快速便利的記錄方式，完整不漏接的服務客戶，並直接與服務的企業數位對接，全面自動化提高服務水平；實時匯集客戶提供的多元數據，透過演算實現精準健康管理服務、推薦客戶最佳保健措施，成為客戶心中之AI健康管理師，徹底領先同業、獲致本平台提供最高性價比的服務。
          </li>
          <li>
            充分適應各式小/中/大型企業之職醫護需求：平台持續跟進職安相關法規變動調整，平台內建自動化數位流程，汲取員工資料同時，即自動進行健康風險分級與納入個案管理機制，讓職醫護即時順利地掌握高風險員工，透過多種管道關懷員工、提升員工健康意識；並配合企業需要，提供各式健康活動所需工具，大大的滿足企業提高照護員工健康成效、降低維護成本，使促進職場健康支持企業永續經營，成就幸福企業。
          </li>
          <li>
            HealthBee
            DHP提供強大的整合運算能力，經由各式標準傳輸協定(如：FHIR)彙整健檢報告、問卷、就醫紀錄/健康存摺及IoT生活量測等數據，依據經實證醫學確認的演算邏輯，提供個人器官系統之關鍵評估報告，適切提出各項建議包括飲食、生活習慣、運動等，專業健康照護者可依個人或群組將適切之健康衛教與提醒服務，透過App推播到客戶/員工手中，亦可經由個人授權，將連續性日常活動數據及健康存摺，供專業健康照護者進行參閱及服務建議。
          </li>
        </ol>
      </>
    ),
  },
  {
    ...
    introduce: (
      <>
        <ol>
          長庚醫科U-Dr遠距醫療產品特色：
          <li>提供一般門診可支援多點連線遠距會診服務應用。</li>
          <li>多元醫儀IoT設備介接服務。</li>
          <li>即時雙通道影音傳輸進行視訊診察應用，醫病親臨互動。</li>
          <li>整合介接各家HIS醫囑系統與遠距會診服務。</li>
        </ol>
        <ol>
          醫療服務面特色優勢：
          <li>
            對民眾端的服務，提供不定點多科別的診療服務，改善偏鄉醫療服務專科別種類及時間的限制，提供醫院對專科醫師服務安排的彈性，醫師看診中即可進行會診，提升醫療服務可近性。
          </li>
          <li>
            對醫師端的服務，提供遠端完整的診療資訊，整合病人視訊影像聲音、五官檢查影像、會診觀察結果，以及歷次重大醫療紀錄等，提升醫療資訊完整性。
          </li>
          <li>
            {" "}
            病歷可彙整於同一醫療資訊系統，不須重複輸入，一次輸入雙方使用，滿足醫療法要求。
          </li>
        </ol>
      </>
    ),
...

  {
    ...
    introduce: (
      <p>
        ...
        打破一成不變體溫量測，整合各種通訊方式如Wi-Fi、4G/LTE等以符合場域通訊需求，以零接觸經濟為主的防疫物聯網架構，可裝設於企業職場、醫療機構、校園機構等場所，使用者於可觸控互動式電腦或平板開啟自動體溫量測App，並使用自動測溫儀器量測，可將量測記錄上傳雲端。透過健保卡、感應員工卡快速識別量測者，並可設定快速量測造冊模式、前台問卷模式等，協助機構把關每位進出者的健康狀況。
...

  {
    ...
    introduce: (
      <ul>
        ...
        它具有先進的設計和人體工程學,使醫護人員能夠快速方便地控制設備。其特點如下:
        <li>15.6吋大尺寸全高清觸控式顯示螢幕</li>
        <li>即時提供準確心電圖採集數據 </li>
        <li>提供心電圖異常診斷達200種</li>
        <li>簡單直觀的使用者輸入操作介面 </li>
        <li>自動將ECG數據傳輸到電子病歷 </li>
        <li>支援外部儲存設備擴充</li>
        <li>支援USB介面印表機</li>
        <li>支援 DICOM 上傳</li>
        <li>設計材質通過抗菌測試</li>
...

  {
    ...
    introduce: (
      <p>
        使用者使用此服務可以透過TWID認證方式來上傳自己的COVID-19疫苗注射紀錄（小黃卡），以及輸入身份證字號來查詢對外合作間接的FHIR
        Server系統找到COVID-19疫苗注射紀錄資料，透過找到的疫苗紀錄來確認使用者注射的資訊之外，還可以讓使用者產生對應的疫苗資訊QRCode，讓使用者可以到店家消費的時候可以出示，當店家讀取QRCode之後，知道使用者目前的疫苗注射紀錄，進而可以讓店家確保與管控使用者的進出，產生QRCode的疫苗護照方式，全程採用不重複與有時效性的權杖（Token）對應的方式進行，QRCode的內容中不會存有使用者敏感資訊。
...

  {
    ...
    introduce: (
      <p>
        FHIR
        智慧醫療數據服務中台為矽塔團隊自主研發，提供無須進行程式撰寫即可將任意醫療資料標準化為
        FHIR
        格式的轉換引擎，協助客戶在跨系統間以標準化格式安全、無痛互通醫療資料與影像／媒體資料，解決系統異質性造成的醫療資訊傳遞品質與效率問題，與傳統
        API 介接相比可提升達 20 倍效率。
      </p>
    ),
  },

  {
    ...
    introduce: (
      <ol>
        <li>
          DocterWatch搭配專屬APP可以使用雲端分析飲食前後的血糖趨勢變化，作為運動飲食/結石/減重等健康飲食管理，目前不提供血糖監測管理，免採血無創血糖演算法已計畫申請臨床試驗準備做精準標準化，未來要申請醫療器材提供糖友免採血無痛無恐懼真正的無創血糖紀錄，雲端分析也包含了心臟健康指數/血管彈性作為日常的健康紀錄。
        </li>
        <li>
          所提供的心律監測為連續監測(每秒上傳伺服器)即時監測心律變化，搭配跌倒監測及求救功能，當伺服器收到異常發報，則會發送異常通知給相關人。
        </li>
        <li>
          搭配連續心律監測、疲勞度、壓力指數、睡眠分析(REM)可以更清楚自己的全天健康狀態，作為更完整的健康紀錄。
        </li>
      </ol>
    ),
  },
  {
    ...
    introduce: (
      <p>
        近年來因應居家照護的蓬勃發展，本公司開發iCare個人健康管理照護平台，讓使用者可以透過LINE
        APP，即時回報個人的健康狀況，並協助醫護人員追蹤居家健康，適時提供衛教資訊或相關照護內容！
        該系統導入FHIR標準，可以快速整合不同醫療數據，進而提供更全面的個人化精準照護服務！
      </p>
    ),
  },
  {
    ...
    introduce: (
      <ol>
        <li>
          DICOM技術整合服務: 提供產生各種標準DICOM物件，例如:
          超音波、心電圖、五官鏡、二次截像、儀器工作清單、DICOM顯示、儲存與傳輸。DICOM去識別化等服務，可針對高度客製化，滿足使用者需求。
        </li>
        <li>
          HL7 FHIR技術整合服務:
          提供臨床資料轉換FHIR格式服務，包含CDA、臨床資料庫等。可針對客戶設計FHIR
          Profile, Implementation Guide,驗證等提供顧問與技術支援。
        </li>
        <li>
          產品: (1) minicoon: DICOM影像儲存與管理 (一般影像以及數位病理影像) (2)
          NHI Uploader: 診所醫療影像上傳健保署。
        </li>
      </ol>
    ),
  },
...
const GovernmentInform = [
  {
    ...
    tracks:
      " 參加賽道 \n TRACK#1 - 病人基本資料互通 \n TRACK#4 - DICOMWeb查詢與調閱 \n TRACK#5 - DICOM數位全幅病理顯微鏡影像查詢與調閱",
    introduce:
      "以前病理醫師在做病理診斷的時候，常常需要找一些很小的病灶，以及對疾病嚴重度做分級。 此程序常常需要耗費許多的眼力、腦力、以及時間。 ALOVAS 數位病理切片影像分析系統利用影像處理的方式與AI人工智慧深度學習與分析，可以幫助病理醫師尋找一些小的病灶，避免遺漏和節省時間。 另外對於疾病的嚴重度，也可以幫助病理醫師做一些客觀且準確的分級，以期達到精準醫療。 \n 本系統為跨平台的系統，醫師可以利用平板或是電腦，將病理影像資料傳送到本系統，目標如下： \n 1. 病理影像分析 \n 2. 腫瘤病理分級 \n 3. 提供細胞相關參數 (核質比、細胞大小、密度分布…等) \n 4. 提供影像分析結果瀏覽 \n 5. 提供醫師影像標註功能 ",
  },
  {
    img: G2,
    ...
    tracks:
      "參加賽道 \n TRACK#1 - 病人基本資料互通 \n TRACK#3 - 用藥及疫苗接種紀錄 \n TRACK#6 - 基因定序標記",
    introduce:
      "臨床研究資訊系統(Clinical Study Information System, CSIS)是一個協助收集臨床研究，或臨床試驗數據與的電子系統。 系統具備強大的設計功能，可以設計一般的國際標準問卷，或客製化的電子化個案報告表(eCRF)。 若由開放電子化個案報告表(eCRF)資料庫所收集的上萬張表單中，挑選eCRF並存為符合ODM之格式，即可直接上傳到使用，免去自行設計的時間。 此外，CSIS亦可設計符合CDISC標準的eCRF，並自動產生註解的個案報告表 (aCRF)以節省人工。 試驗計畫(study plan)中可列出受試者每次來訪所需的表單，並以儀表版呈現表單填寫的狀態，方便了解收案進度。",
  },
  {
    img: G2,
